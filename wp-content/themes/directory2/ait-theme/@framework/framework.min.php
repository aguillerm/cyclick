...
function
embedOrEnqueueCssGeneratedFromLess($handler,$output){if($output['isEmpty'])return;if($output['error']){wp_add_inline_style($this->lastEnqueuedCssHandler,$output['embedCss']);}else{$this->lastEnqueuedLessHandler=$handler;wp_enqueue_style($handler,$output['url'],array(),$output['version']);}}protected
function
...
function
addLangToCustomWpQuery($query,$originalArgs){global$polylang;if(isset($query['post_type'])){$translatableCpts=get_post_types(array('ait-translatable-cpt'=>true));if(isset($query['tax_query'])and
